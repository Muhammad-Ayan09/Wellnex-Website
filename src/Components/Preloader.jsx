...
const Preloader = ({ loading }) => {
  ...
  useEffect(() => {
    if (loading) {
      ...
      // rotating dot around the circle
      if (rotorRef.current) {
        gsap.set(rotorRef.current, { rotate: 0, transformOrigin: "50% 50%" });
        gsap.to(rotorRef.current, {
          rotate: 360,
          duration: 3,
          ease: "power2.inOut",
        });
        // pulse while rotating
        const dot = rotorRef.current.querySelector('.pulse-dot');
        if (dot) {
          gsap.to(dot, {
            scale: 1.25,
            repeat: -1,
      ...
      <div className="w-full max-w-xl px-6 text-center select-none">
        ...
        <div className="mt-8 flex items-center justify-center">
          <div className="relative w-32 h-32">
            ...
            {/* rotating dot */}
            <div ref={rotorRef} className="absolute inset-0 grid place-items-center">
              <div className="relative" style={{ width: 0, height: 0 }}>
                <div className="pulse-dot absolute" style={{ left: 60, top: -48 }}>
                  <div className="w-3 h-3 rounded-full bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.7)]" />
                </div>
              </div>
            </div>
          </div>
